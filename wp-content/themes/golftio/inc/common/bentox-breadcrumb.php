...
function bentox_breadcrumb_func()
{
    ...
    if (is_front_page() && is_home()) {
        $title = get_theme_mod('breadcrumb_blog_title', __('Blog', 'golftio'));
        ...
        $title = get_theme_mod('breadcrumb_product_details', __('Shop', 'golftio'));
    } elseif (is_single() && 'courses' == get_post_type()) {
        $title = esc_html__('Course Details', 'golftio');
    } elseif ('courses' == get_post_type()) {
        $title = esc_html__('Courses', 'golftio');
    } elseif (is_search()) {
        $title = esc_html__('Search Results for : ', 'golftio') . get_search_query();
    } elseif (is_404()) {
        $title = esc_html__('Page not Found', 'golftio');
    } elseif (function_exists('is_woocommerce') && is_woocommerce()) {
        $title = get_theme_mod('breadcrumb_shop', __('Shop', 'golftio'));
    } elseif (is_archive()) {
    ...
    <div class="search-wrapper p-relative transition-3 d-none">
        <div class="search-form transition-3">
            <form method="get" action="<?php print esc_url(home_url('/')); ?>">
                <input type="search" name="s" value="<?php print esc_attr(get_search_query()) ?>" placeholder="<?php print esc_attr__('Enter Your Keyword', 'golftio'); ?>">
