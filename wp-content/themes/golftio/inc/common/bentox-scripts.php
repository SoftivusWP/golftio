...
function bentox_scripts() {
     ...
     * all css files
    */
    wp_enqueue_style( 'golftio-fonts', 'https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&display=swap' );
    ...
    // Glyphter
    wp_enqueue_style('glyphter', get_template_directory_uri() . '/assets/vendor/glyphter/css/golftio.css');
    ...
    wp_enqueue_style( 'spacing', GOLFTION_THEME_CSS_DIR . 'spacing.css', [],time() );
    wp_enqueue_style( 'main-css', GOLFTION_THEME_CSS_DIR . 'main.css', [],time() );
    wp_enqueue_style( 'golftio-blog-css', GOLFTION_THEME_CSS_DIR . 'golftio-blog.css', [],time() );
    wp_enqueue_style( 'woocommerce-css', GOLFTION_THEME_CSS_DIR . 'woocommerce.css', [],time() );
    wp_enqueue_style( 'golftio-unit', GOLFTION_THEME_CSS_DIR . 'golftio-unit.css', [], time() );
    wp_enqueue_style( 'golftio-custom', GOLFTION_THEME_CSS_DIR . 'golftio-custom.css', [],time() );
    wp_enqueue_style( 'responsive', GOLFTION_THEME_CSS_DIR . 'responsive.css', [],time() );
    wp_enqueue_style( 'golftio-style', get_stylesheet_uri() );
...
function bentox_fonts_url() {
     ...
     */
    if ( 'off' !== _x( 'on', 'Google font: on or off', 'golftio' ) ) {
